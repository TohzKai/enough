...
const zhSG = {
  ...
  home: {
    ...
    heroChild:
      "帮父母安心规划退休支出。一次性连接他们的账户，通过提醒持续关注，并在重大变动时由您会签——而数字始终由他们掌控。",
    ctaParent: "开始我的规划",
    ctaChild: "为父母设置",
    seeExample: "查看示例",
    ...
    pillarFamilyBody:
      "为退休人、配偶和成年子女提供权限化规划——子女协助搭建，但数字始终由长辈确认。",
  ...
  results: {
    ...
    oversightTitle: "监督而不打扰——爸爸仍确认每个数字",
    goToFamily: "前往家庭规划会签 →",
    // Section title
    kickerParent: "您的结果",
    kickerChild: "您父母的结果",
    ...
    nextTitleChild: "邀请家人一起会签",
    nextBodyParent: "打开一份从容、可打印的一页报告，在家分享。",
    nextBodyChild: "在家庭规划上审阅并会签这次安全上调——爸爸确认。",
  ...
  family: {
    ...
    childTag: "监督而不打扰",
    parentBody:
      "您的子女可以协助您搭建并保持关注，但他们绝不能在未经您同意的情况下动用您的资金或更改您的数字。您看到并确认每一项变动。规划始终站在您这边。",
    childBody:
      "您搭建并连接账户，在规划需要关注时收到提醒，并会签重大变动——但最终数字始终由爸爸确认。这条以长辈为中心的规则，正是 Enough 值得信赖的根基。",
    whoOnPlan: "参与规划的人",
    coSignerHeading: "会签流程",
    coSignerIntro:
      "重大变动——上调支出、CPF 补足、连接账户——通过共享审批进行。操作者可发起与审阅；拥有者确认。",
    moatTitle: "家庭层级是不可争议的护城河",
    moatBody:
      "在我们所调研的新加坡及全球竞争对手中，没有任何一家提供此功能。它解决了年长者数据录入难题（由子女操作）、修复了付费意愿问题（由操心的子女付费），并形成了计算器无法复制的家庭内部纽带。",
    openReport: "打开家庭报告 →",
    backToResults: "返回结果",
    disclaimer:
      "家庭层级的示意性原型。角色、权限与会签流程以示例数据演示——并非真实账户。",
    // member labels
    m1Name: "Mr Tan",
    m1Relation: "退休人",
    m1Role: "拥有者 · 决策者",
    ...
    m3Relation: "成年女儿",
    m3Role: "操作者 · 副驾驶",
    m3p1: "代表爸爸（经其授权）搭建并连接账户",
    m3p2: "在规划需要关注时收到提醒",
    m3p3: "会签重大变动——但最终数字由爸爸确认",
    // cosign
    csAwaitingParent: "等待爸爸确认",
    csAwaitingChild: "等待审阅 + 确认",
    csApproved: "已批准",
    csRaisedBy: "发起人",
    csNeeds: "需要",
    csParentCentric: "以长辈为中心：",
    csConfirmAsDad: "确认（作为爸爸）",
    csReviewConfirm: "审阅并确认",
    csNotNow: "暂不",
  },

  familyPlane: {
    cs1Title: "将安全月支出上调至 S$2,350",
    cs1Detail:
      "市场走强后，护栏获得了一次上调。Wei Ling 已审阅；在生效前由爸爸确认。",
    cs1RaisedBy: "Wei Ling（操作者）",
    cs1Needs: "须由 Mr Tan 确认",
    cs1Note:
      "Enough 始终以长辈的福祉为优先——这次上调是安全的，所以规划把它呈现给爸爸，而不是绕过他。",
    cs2Title: "假设更高的 CPF LIFE 保底",
    cs2Detail:
      "更高的保底收入会提升概率。我们建议方向；具体产品由爸爸决定——而且由爸爸拍板。",
    cs2RaisedBy: "Enough（护栏引擎）",
    cs2Needs: "Mr Tan + Wei Ling",
    cs3Title: "通过 SGFinDex 连接 OCBC SRS 账户",
    cs3Detail: "Wei Ling 已连接 SRS 账户，以便提取顺序可以利用 10 年窗口期。",
    cs3RaisedBy: "Wei Ling（操作者）",
    cs3Needs: "经 Mr Tan 授权",
    alert1Title: "爸爸的规划进展顺利",
    alert1Body: "至 95 岁仍有 90% 概率。无需行动。",
    alert2Title: "有一次安全上调正等待爸爸确认",
    alert2Body: "市场已高于规划线——安全月支出可上调至 S$2,350。",
    alert3Title: "有一项 CPF 补足决策需要一起审视",
    alert3Body: "更高的保底会把概率提升约 3%。我们建议方向，不针对特定产品。",
  },
